...
class MembersController extends AppController {
    ...
    public function approve_registration($id){
        ...
        $last_tag_n = $this->YouthConference->find('first',array('conditions'=>array('YouthConference.verified'=>1),'order'=>array('YouthConference.id DESC'),'fields'=>array('YouthConference.tag')));
        $delegate = $this->YouthConference->find('first',array('conditions'=>array('YouthConference.id'=>$id)));
        if(empty($last_tag_n)){
            $last_tag_n = 0;
            $new_tag_no = $last_tag_n + 1;
        }else{
            $last_tag_no = $last_tag_n['YouthConference']['tag'];
            $new_tag_no = $last_tag_no + 1;
        }
        $group = array(
            'Ibraheem Hakeem',
            'Badmus Kamorudeen',
